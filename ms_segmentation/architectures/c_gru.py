...
#
# Description:  Script that defines convolutional Gatet Recurrent Unit (ConvGRU)
#
# Author:       Sergio Tascon Morales (Research intern at mediri GmbH, student of Master in Medical Imaging and Applications - MAIA)
#
# Details:      From https://github.com/happyjin/ConvGRU-pytorch 
#
# --------------------------------------------------------------------------------------------------------------------
...
class ConvGRUCell(nn.Module):
    ...
    def forward(self, input_tensor, h_cur):
        ...
        h_next = (1 - update_gate) * cnm + update_gate * h_cur
        return h_next
...
class ConvGRUCell3D(nn.Module):
    def __init__(self, input_size, input_dim, hidden_dim, kernel_size, bias, dtype):
        ...
        :param input_size: (int, int, int)
            Height and width of input tensor as (height, width, depth).
        :param input_dim: int
            Number of channels of input tensor.
        :param hidden_dim: int
            Number of channels of hidden state.
...
class ConvGRU3D(nn.Module):
    ...
    def forward(self, input_tensor, hidden_state=None):
        ...
        for layer_idx in range(self.num_layers):
            ...
            for t in range(seq_len):
                # input current hidden and cell state then compute the next hidden and cell state through ConvLSTMCell forward function
                h = self.cell_list[layer_idx](input_tensor=cur_layer_input[:, t, :, :, :, :], # (b,t,c,h,w,d)
                                              h_cur=h)
                output_inner.append(h)
